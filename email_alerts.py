...
def get_next_alert_date():
    """
    Returns the next alert_date that should be processed,
    based on email_alerts_log (source of truth).
    """
    query = """
...
def log_email_alert(alert_date, red_df, green_df, partner_df, csv_paths):
    red_csv = None
    green_csv= None
    partner_csv = None
    ...
    query = f"""
        ...
        (alert_date, red_alert_count, green_alert_count, partner_alert_count, csv_files_count,
         red_alert_csv, green_alert_csv, partner_spike_csv, sent_at)
        VALUES
        ('{alert_date}', {red_count}, {green_count}, {partner_count}, {csv_files_count},
...
def main():
    ...
    # STEP 1: Get RED alerts
    all_red_alerts, combined_red_alerts, all_green_alerts, combined_green_alerts, partner_spikes, alert_date = get_alerts()

    if alert_date is None:
        print("🛑 No valid alert date. Exiting.")
        return
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            try:
                log_email_alert(alert_date, all_red_alerts, all_green_alerts, partner_spikes, attachments)
